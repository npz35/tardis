...
from app.data_model import LLMResponse, TranslationResponse
import time
import traceback
import tempfile
...
class Translator:
    ...
    def translate_texts(self, texts: list[str], source_lang: str = 'English',
                        target_lang: str = 'Japanese', max_retries: int = 3) -> list[TranslationResponse]:
        ...
        initial_result: TranslationResponse = TranslationResponse(
        ...
        if not texts:
            ...
            result: TranslationResponse = copy.deepcopy(initial_result)
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                llm_responses: list[LLMResponse] = self.llm.translation_request(payload)
                ...
                for i, llm_response in enumerate(llm_responses):
                    ...
                    result.tokens_used = llm_response.response_json.get('prompt_eval_count', 0) + llm_response.response_json.get('eval_count', 0)
                    result.attempts = attempt + 1
    ...
    def _is_invalid_translation(self, original_text: Optional[str], translated_text: str) -> bool:
