...
from app.data_model import BBox, FontInfo, TextPermutation
...
class PdfTextLayout:
    ...
    def draw_translated_text(self, c: canvas.Canvas, translated_text: str, bbox: BBox, font_info: FontInfo) -> None:
        ...
        current_y = bbox.y1 - optimal_font_size * Config.JP_CHAR_HEIGHT_FACTOR

        for line in lines:
            # if current_y < bbox.y0: # Check if we are out of bounds
            #     self.logger.warning(f"Text line '{line}' exceeds bounding box height. Skipping.")
            #     break
            self.logger.debug(f'Draw line: {line}, bbox: {bbox}')
            c.drawString(bbox.x0, current_y, line)
    ...
    def draw_white_rectangle(self, c: canvas.Canvas, bbox: BBox) -> None:
        ...
        c.rect(bbox.x0, bbox.y0, width, height, fill=1, stroke=0) # Fill with white, no border
    ...
    def _expand_bbox(self, bbox: BBox) -> BBox:
        ...
        '''
        expanded_bbox = BBox(x0=bbox.x0 - Config.BBOX_EXPAND_MARGIN, y0=bbox.y0 - Config.BBOX_EXPAND_MARGIN, x1=bbox.x1 + Config.BBOX_EXPAND_MARGIN, y1=bbox.y1 + Config.BBOX_EXPAND_MARGIN)
        self.logger.debug(f'Function end: _expand_bbox (success) -> {expanded_bbox}')
        return expanded_bbox
    def draw_multiple_translated_texts_in_area(
        self,
        c: canvas.Canvas,
        ...
        """
        Draw multiple translated texts within a single TextArea without overlap.
        Texts are arranged vertically from top to bottom.
        """
        self.logger.debug(f'Function start: draw_multiple_translated_texts_in_area(translated_texts_len={len(translated_texts)}, bbox={bbox}, font_info={font_info})')
        width = bbox.x1 - bbox.x0
        height = bbox.y1 - bbox.y0
    
        if width <= 0 or height <= 0:
            self.logger.error(f'Invalid bounding box dimensions: width={width}, height={height}')
            self.logger.debug('Function end: draw_multiple_translated_texts_in_area (failed - invalid bbox)')
            raise ValueError('Invalid bounding box dimensions for drawing text.')
    
        font_size = font_info.size
        if font_size <= 0:
            self.logger.error(f'Invalid font size: {font_size}')
            self.logger.debug('Function end: draw_multiple_translated_texts_in_area (failed - invalid font size)')
            raise ValueError('Invalid font size for drawing text.')
    
        # Adjust font size to fit
        optimal_font_size = self._adjust_font_size_to_fit_multiple(c, translated_texts, width, height, font_size)
        ...
        # Calculate initial y position (top of the box minus a margin)
        current_y = bbox.y1 - optimal_font_size * Config.JP_CHAR_HEIGHT_FACTOR
    
        for text in translated_texts:
            lines = self._calculate_text_lines(c, text, width, optimal_font_size)
        ...
        self.logger.info(f'Finished drawing translated text. Final font size: {optimal_font_size}')
        self.logger.debug('Function end: draw_multiple_translated_texts_in_area (success)')
    
    def _adjust_font_size_to_fit_multiple(self, c: canvas.Canvas, texts: list[str], max_width: float, max_height: float, initial_font_size: float) -> float:
        self.logger.debug(f'Function start: _adjust_font_size_to_fit_multiple(texts_len={len(texts)}, max_width={max_width}, max_height={max_height}, initial_font_size={initial_font_size})')
        '''
        Adjusts font size to ensure multiple texts fit within the given width and height.
        '''
        current_font_size = initial_font_size
        for _ in range(10): # Try up to 10 iterations for adjustment
        ...
        self.logger.debug(f'Function end: _adjust_font_size_to_fit_multiple (success) -> {current_font_size}')
        return current_font_size
    
    def layout_bboxes(self, extracted_text_data: list[TextPermutation]) -> list[TextPermutation]:
        # extracted_text_data[i].bbox 同士が重なり合わないように調整する
        # 重なり合う2つ以上の extracted_text_data[i] を1つの TextPermutation 型インスタンスへ統合する
        pass