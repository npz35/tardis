...
class TestTranslatorIntegration(unittest.TestCase):
    ...
    def test_translate_text_real_api_success(self):
        ...
        try:
            ...
            self.assertTrue(health_check_result.success, f"API health check failed: {health_check_result.error}")
            # Also check if the model exists (whether the target model is available)
            # self.assertTrue(health_check_result.model_available, f"Model '{self.translator.model}' not available. Available models: {health_check_result.available_models}")
        ...
        # 結果の検証
        self.assertTrue(result[0].success, f"Translation failed: {result[0].error}")
