...
class TestAreaColoringEndpoint:
    def test_area_coloring_endpoint(self, client):
        ...
        # 2. Call /area_separation with the obtained filename and unique_id
        separation_data = {
            'filename': filename,
            'unique_id': unique_id
        }
        response = client.post('/area_separation', data=separation_data)
    ...
    def test_area_coloring_endpoint_success(self, client):
        file_path = 'samples/sample.pdf'
    ...
    def test_area_coloring_endpoint_no_file(self, client):
        response = client.post('/area_separation', data={})
        assert response.status_code == 302 # Redirect to index
        with client.session_transaction() as session:
            assert 'An error occurred during area coloring' in session['_flashes'][0][1]

    def test_area_coloring_endpoint_empty_filename(self, client):
        response = client.post('/area_separation', data={'filename': '', 'unique_id': 'some_id'})
        assert response.status_code == 302 # Redirect to index
        with client.session_transaction() as session:
            assert 'An error occurred during area coloring' in session['_flashes'][0][1]

    def test_area_coloring_endpoint_invalid_extension(self, client):
        file_path = 'samples/sample.pdf'
        with open(file_path, 'rb') as f:
    ...
    def test_area_coloring_endpoint_file_too_large(self, client, tmp_path):
    ...
    def test_area_coloring_endpoint_disk_space_error(self, client):
        file_path = 'samples/sample.pdf'
        ...
        # Flash message is not set for 503 errors
        # with client.session_transaction() as session:
        #     assert 'Insufficient disk space. Please free up some space.' in session['_flashes'][0][1]

    def test_area_coloring_endpoint_area_coloring_error(self, client):
        file_path = 'samples/sample.pdf'
        ...
        # 2. Call /area_separation with the obtained filename and unique_id, and mock the separator
        with patch('app.main.PdfAreaSeparator') as MockPdfAreaSeparator:
            ...
            mock_instance.extract_area_infos.return_value = [] # ダミーのページ情報なのだ
